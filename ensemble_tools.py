...
datapath = '/home/magnus/DMI_ensembles/'
modulepath = '/home/magnus/dmi_ensemble_handler/'
...
def save_most_recent_timeseries(fieldname, ts_start, ts_end, pointcode=71699, \
    ...
    np.save(modulepath+savepath+filename, ens_timeseries)
...
def save_ens_mean_avail_at10_series(ts_start, ts_end, pointcode=71699):
    ...
    for v in ['Tout', 'hum', 'vWind', 'sunRad']:
        try:
            ens_data = np.load(modulepath + 'time_series/avail_at10_thedaybefore/' + v + suffix)
        except:
        ...
        np.save(modulepath+savepath+v+suffix, hourly_mean)
...
def load_ens_avail_at10_series(ts_start, ts_end, varname, pointcode=71699):
    suffix = ''.join(['_geo', str(pointcode), '_', timestamp_str(ts_start), \
                        '_to_', timestamp_str(ts_end), '.npy'])
    try:
        ens_data = np.load(modulepath+'time_series/avail_at10_thedaybefore/' + varname + suffix)
    except:
        print "Ensemble times series not found. Generating more: "
...
def load_ens_mean_avail_at10_series(var, ts_start, ts_end, pointcode=71699):
    ...
    suffix = ''.join(['_geo', str(pointcode), '_', timestamp_str(ts_start), \
                        '_to_', timestamp_str(ts_end), '.npy'])
    try:
        ens_mean = np.load(modulepath+load_path + var + suffix)
    except:
...
def save_ens_mean_series(ts_start=dt.datetime(2015,12,16,1),\
    ...
    for v in ['Tout', 'hum', 'vWind', 'sunRad']:
        try:
            ens_data = np.load(modulepath + 'time_series/' + v + load_suffix)
        except:
            print "Ensemble times series not found. Generating more: "
        ...
        np.save(modulepath + savepath + v + save_suffix, hourly_mean_excepday1)
        np.save(modulepath + savepath + v + 'avg24' + save_suffix, mean_last_24h)
        print "Saved files: ", modulepath + savepath + v + save_suffix + "\n" + savepath + v + 'avg24' + save_suffix
...
def repack_ens_mean_as_df(ts_start=dt.datetime(2015,12,17,1), ts_end=dt.datetime(2016,1,15,0),\
    ...
    data_dict = {v:np.load(modulepath + load_path + v + load_suffix) for v in allvars}
